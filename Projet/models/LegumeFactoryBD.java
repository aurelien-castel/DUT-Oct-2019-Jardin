...
public class LegumeFactoryBD extends  MethodeBD implements AbstractLegumeFactory {
    ...
    public static LegumeFactoryBD getLegumeFactoryBD(){ 
     ...
     private Legume newLegume(int id,String nomLegume, FamilleLegume famille, int semis, int recolte ){
        return new LegumeBD(id,nomLegume,famille,semis,recolte);
    }
    ...
    public Legume AddLegume(String nomLegume, FamilleLegume famille, int semis, int recolte){
        ...
        int id =0;
        Legume test = this.searchlegume(nomLegume);
        Legume legume = null; 
        if (test == null) {
            setConnection();
            ...
            // ici retourner un Legume
            legume = this.newLegume(id, nomLegume,famille,semis,recolte);
            return legume;
    ...
    public Iterator<Legume> getLegumes(){
        throw new UnsupportedOperationException("Désolé, fonction non supporté utilisé plutôt getallLegume().");
    }

    // cherche légumes connus par famille.
    public Iterator<Legume> getLegumes(FamilleLegume famille){
        //String sql "SELECT * FROM `Legume` WHERE `familleLegume` = ?;"
    ...
    private Legume searchlegume(String mot){
        ...
        if (vide == true) {
            ...
            legume = this.newLegume(id, nomLegume,familleLegumename,semis,recolte);
            return legume ;
        }
    }
